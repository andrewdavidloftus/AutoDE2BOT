...
; Maybe just have a function that we store expected left, right, and travel distances
; as well as direction to turn, side of expected trigger, and distance for that as well
; Then all we have to do is have it move as far as it should while checking for those distance
...
DriveLoop: OUT RESETPOS
  ...
  ;;ADD ALL DRIVESCAN CALLS AND LOOP AT THE END
  ; maybe a direction value and add the variable baffle distance here too?
...
;* Arguments
;***************************************************************
LEFTD:    DW 0   ;Distance it should be able to see left
RIGHTD:   DW 0   ;Distance it should be able to see right
BUFDIST:  DW 0   ;Distance it will maintain from the wall it is tracking (SHOULDNT THIS BE THE SAME AS THE DISTANCE IT CAN SEE TO THE WALL ITS TRACKING???)
EXSIDE:   DW 0   ;Trigger to move to next mode -1 for left, 1 for right, 0 for odom | also sets which wall to track
TRIGGERD: DW 0   ;Distance to trigger next mode by sonar
TRIGGERT: DW 0   ;1 for when sonar goes close to farther, -1 for opposite
...
;;I should probably add some sort of safety code for this huh
;;Currently will just drive the U shape around the baffle
DriveScan:
	LOADI &B00100001 ;Enable Sensors 0 and 5
	OUT SONAREN

;;shamelessly stolen
	LOADI  0
	STORE  DTheta      ; Desired angle 0
	LOADI  511         ; Load velocity (very fast)
	STORE  DVel        ; Desired forward velocity

  ;;not stolen
  LOADI 300          ; Distance to check
  STORE RANGE
  ;;----------

Driving:
	LOADI 511     ;Keep going fast
	STORE  DVel

  ;DEBUG STUFF --------------
  IN DIST0      ;Read Sonar 0
	OUT SSEG1     ;Left SSeg
  IN DIST5      ;Read Sonar 5
  OUT SSEG2     ;Right SSeg
	IN XPOS       ;Read odom x
	OUT LCD       ;Main LCD
  ;--------------------------
...
GoSet: LOADI 5   ;I think this is .5 seconds
  STORE RANGE
  IN TIMER
  JNEG SkipTrack
  ...
  ;track wall
  ;Maybe change this so that it tracks the change in the tracked distance and tries to minimize it
CheckClose: LOAD BUFDIST
  ADDI -100      ;Off by 10 cm
  STORE RANGE
  CALL CheckRange
  JPOS CheckFar

  ;too close to wall
  LOAD EXSIDE ;turn a little away from the wall
  SHIFT 2
  ADD THETA
  STORE DTHETA
  JUMP SkipTrack

CheckFar: LOAD BUFDIST
  ADDI 100      ;Off by 10 cm
  STORE RANGE
  CALL CheckRange
  JNEG SkipTrack
  ...
  ;too far from wall
  XOR EXSIDE ;turn a little towards the wall (need to invert direction)
  ADDI 1
  SHIFT 2
  ADD THETA
  STORE DTHETA
...
SkipTrack: LOAD TRIGGERD
  STORE RANGE
  LOAD WALLD
  CALL CheckRange
  ADD TRIGGERT        ;Adds 1 and -1 when trigger happens
  JZERO TriggerHit


  JUMP Driving      ;Keep looping
...
TriggerHit: OUT RESETPOS

	LOADI  0
	STORE  DTheta      ; Desired angle 0

DrivePassed: LOADI 511
  STORE DVel
  LOADI 300     ;Drive 30 cm ~ 1 foot
  STORE RANGE
  IN XPOS       ;Read odom x
  CALL CheckRange
  JNEG DrivePassed
